...
module.exports = {
    ...
    getPublished: async (req, res) => {
        try {
            ...
            if(!publishedPosts.length) res.status(404).json(`found ${publishedPosts.length} published Posts`)
            return res.status(200).json({success: true, noOfPublishedPosts: publishedPosts.length, posts: publishedPosts})
        } catch {
    ...
    publishPost: async (req, res) => {
        try {
            ...
            if(post.status === "published") return res.status(400).json({success: false, message: "post is already published"})
    ...
    unpublishPost: async (req, res) => {
        try {
            const post = await Post.findById(req.params.id)
            if(post.status === "unpublished") return res.status(400).json({success: false, message: "post is already unpublished"})
    ...
    getComments: async (req, res) => {
        try{
            ...
            return res.status(200).json({success: true, noOfComments: comments.length, comments: comments})
        } catch (err) {
            console.log(err)
